...
import { handleQuestionFormatChange, handleAddQuestion, handleAddTeamModalSubmit, handleRemoveQuestion, handleBackToAddTeam} from "../actionHandlers/addTeamModalHandler";
import {
  OverflowAction,
  BlockAction,
  BlockElementAction,
...
export const addTeamModal_action = (app: App) => {
  ...
  app.action('question_format', handleQuestionFormatChange);
  // app.action('preview_question', handleQuestionPreview);
  // Handle block actions, specifically the "Back" button
  // app.action('back_to_add_team', handleBackToAddTeam);
  app.action({ action_id: 'add_question', type: 'block_actions' }, handleAddQuestion);
  
  app.action({ action_id: /^remove_question_/,type: 'block_actions' }, handleRemoveQuestion);
  ...
  //   try {
  //     const viewState = (body as unknown as BlockAction).view?.state?.values;
  //     // const standupQuestions =
  //     //   viewState?.standup_questions_block?.standup_questions?.value || "";

  //     const blocks = (body as BlockAction).view?.blocks;

  //     const standupQuestions = blocks?.reduce((acc: any[], block: any) => {
  //       const blockId = block.block_id;

  //       // Extract question input
  //       if (blockId?.endsWith("_question")) {
  //         const sharedPrefix = blockId.substring(
  //           0,
  //           blockId.lastIndexOf("_question")
  //         );
  //         const questionInput = (body as BlockAction).view?.state.values[
  //           blockId
  ...
  //         // find corresponding format block
  //         const formatBlockId = `${sharedPrefix}_format`;
  //         const formatValue = (body as BlockAction).view?.state.values[
  //           formatBlockId
  //         ]?.format_select?.selected_option?.value;

  //         console.log("Format:", formatValue);

  //         acc.push({ question: questionInput, format: formatValue });
  //       }
  //       return acc;
  //     }, []);

  //     const standupDays =
  //       viewState?.standup_day_block?.standup_days?.selected_options?.map(
  //         (o: any) => o.value
  //       ) || [];
  //     const standupTimes =
  //       viewState?.standup_time_block?.standup_times?.selected_options?.map(
  //         (o: any) => o.value
  //       ) || [];
  //     const timeZone =
  //       viewState?.time_zone_block?.time_zone?.selected_option?.value;
  //     // const reminderDays = viewState?.reminder_day_block?.reminder_days?.selected_options?.map((o: any) => o.value) || [];
  //     const reminderTimes =
  //       viewState?.reminder_time_block?.reminder_times?.selected_options?.map(
  //         (o: any) => o.value
  //       ) || [];

  //     // Logging the values
  //     // console.log("Payload:", JSON.stringify(body, null, 2));
  //     console.log("View State:", JSON.stringify((body as BlockAction).state));
  ...
  //       questions: JSON.stringify(standupQuestions),
  //       standupDays,
  //       standupTimes,
  //       timeZone,
  //       reminderTimes,
  ...
  //     const savedStandupsIndex: number | undefined = currentBlocks?.findIndex(
  //       (block: any) => block.block_id === "saved_standups_section"
  //     );
  ...
  //       console.log(
  //         "Inside savedStandupsIndex && savedStandupsIndex !== -1  && currentBlocks"
  //       );
  //       currentBlocks.splice(savedStandupsIndex, 0, successMessageBlock); // Insert before

  //       let savedStandupText = "Saved Standups:\n";
  //       savedStandupText += `• Questions: ${newStandup.questions}\n`;
  //       savedStandupText += `• Days: ${newStandup.standupDays.join(", ")}\n`;
  //       savedStandupText += `• Time: ${newStandup.standupTimes.join(", ")} ${
  //         newStandup.timeZone
  //       }\n`;
  //       savedStandupText += `• Reminders: ${newStandup.standupDays.join(
  //         ", "
  //       )} at ${newStandup.reminderTimes.join(", ")}`;

  //       const existingSavedStandupsBlock = currentBlocks?.find(
  //         (block) => block.block_id === "saved_standups_section"
  //       ) as SectionBlock;
  ...
  //       title: (body as BlockAction).view?.title || {
  //         type: "plain_text",
  //         text: "Standup Settings",
  //       },
  //       submit: (body as BlockAction).view?.submit || {
  //         type: "plain_text",
  //         text: "Save",
  //       },
  //       close: (body as BlockAction).view?.close || {
  //         type: "plain_text",
  //         text: "Cancel",
  //       },
  //       blocks:
  ...
  //         standups: [
  //           ...JSON.parse(
  //             (body as BlockAction).view?.private_metadata || '{"standups":[]}'
  //           ).standups,
  //           newStandup,
  //         ],
  //       }),
  ...
  //     const teamName =
  //       view.state.values?.team_name_block?.team_name?.value || "";
  //     const memberNames =
  //       view.state.values.team_members_block.team_members.selected_users;

  //     const timeZone =
  //       viewState?.time_zone_block?.time_zone?.selected_option?.value;

  //     var teamStandUp = JSON.parse(body.view.private_metadata).standups;

  //     console.log('Before:',teamStandUp)

  //     // const StandUpParse = teamStandUp.map((item)=> item.)

  //     teamStandUp = teamStandUp.map((item: any) => {
  ...
  //     console.log("TeamStandup After:", teamStandUp);

  //     if (!memberNames || memberNames.length === 0) {
  //       errors["team_members_block"] =
  ...
  app.view('add_team_modal', handleAddTeamModalSubmit);
};
...
export const overflowMenu_action = (app: App) => {
  ...
  app.action<BlockAction<OverflowAction>>(
    ...
    async ({ ack, body, client }) => {
      ...
      console.log(
        "Overflow Action Body Selected Option",
    ...
    if ("action_id" in action) {
      try {
        ...
        const teamId = JSON.parse(
          (body as BlockAction<OverflowAction>).view?.private_metadata || ""
        ).teamId; // Ensure private_metadata contains the team ID
        ...
        await updateTeamByID(teamId, {
          teamstandupQuestions: team.teamstandupQuestions,
        });
        ...
        console.log(
          `✅ Standup ${standupIndex + 1} deleted for team ${teamId}`
        );
      } catch (error) {
    ...
    async ({ ack, action, body, client }) => {
      ...
      try {
        const lastUnderscoreIndex =
          action.selected_option.value.lastIndexOf("_");
        const actionType = action.selected_option.value.substring(
          0,
          lastUnderscoreIndex
        );
        const memberId = action.selected_option.value.substring(
          ...
          if (!team.admins.includes(memberId)) {
            ...
            try {
              ...
              try {
                ...
                isUserInChannel =
                  membersResponse.members?.includes(memberId) || false;
              } catch (error) {
        ...
        await publishEditTeamView(client, body.user.id, teamId);
        console.log(
          `Action ${actionType} completed successfully for member: ${memberId}`
        );
      } catch (error) {
        console.error("Error handling overflow action:", error);
      }
      ...
      try {
        ...
        if (selectedAction && "selected_user" in selectedAction) {
          ...
          try {
            ...
            console.log(
              `Successfully added user ${newUserId} to channel ${team.teamId}`
            );
            ...
            if (inviteError.data?.error === "already_in_channel") {
              ...
              console.warn(
                "Failed to add user to the channel for another reason."
              );
            }
          }
...
//       const questions =
//         view.state.values?.questions?.input?.value?.split("\n") || [];
//       const reminderTimes =
//         view.state.values?.reminderTimes?.input?.selected_options?.map(
//           (option) => option.value
//         ) || [];
//       const standupDays =
//         view.state.values?.standupDays?.input?.selected_options?.map(
//           (option) => option.value
//         ) || [];
//       const standupTimes =
//         view.state.values?.standupTimes?.input?.selected_options?.map(
//           (option) => option.value
//         ) || [];
  ...
  app.view("delete_team_modal", async ({ ack, body, view, client }) => {
    ...
    try {
      ...
      if (!channelResponse.ok || !channelResponse.channel) {
        throw new Error(
          "Channel associated with the teamId could not be found."
        );
      }
